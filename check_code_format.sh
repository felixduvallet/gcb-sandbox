...
FILES_TO_CHECK=$(git diff --name-only origin/master | grep -E ".*\.(cpp|cc|c\+\+|cxx|c|h|hpp)$")
...
FORMAT_DIFF=$(git diff -U0 origin/master -- ${FILES_TO_CHECK} | python ./tools/clang-format-diff.py -p1 -style=file)
